...
and replaces `CalDAVClient.prototype.tryDiscoveryRoots` to ask the base URL before the discovery probes,
`CalDAVClient.prototype.mkIcsPut` to write without following redirects, and adds
`CalDAVClient.prototype.deleteHref` to delete an object at its own address — check these after an update.
