...
the `.mcpb`, each with its own licence file. caldav-mcp uses `@modelcontextprotocol/sdk` (MIT), `ts-caldav` (ISC) and
`zod` (MIT) directly. For 1.1.4 that came to 117 packages: 105 MIT, 8 ISC, 2 BSD-3-Clause, 1 BSD-2-Clause and
`ical.js` 2.2.1 under MPL-2.0, shipped unchanged (source: [kewisch/ical.js](https://github.com/kewisch/ical.js)).

`apps/server/index.js` reaches into `ts-caldav` (`node_modules/ts-caldav/dist/index.mjs`) to raise its request
timeout — check that path after an update.
